...
const scrollbarStyle = `
  ...
  .nb-scroll::-webkit-scrollbar-thumb:hover { background: rgba(37,99,235,0.55); }
  .nb-scroll { scrollbar-width: thin; scrollbar-color: rgba(37,99,235,0.28) transparent; }
`;
...
function NoticeRow({ item }) {
  return (
    <div className="group flex items-start gap-3 py-3 border-b border-soft/50 last:border-0 cursor-pointer hover:bg-[#F1F5F9] transition-colors duration-200 px-4 -mx-4 rounded">
      <div className="flex-shrink-0 mt-0.5">
        <div className="w-5 h-5 rounded-full border-2 border-brand-blue flex items-center justify-center group-hover:bg-brand-blue transition-colors duration-200">
          <ChevronRight size={10} className="text-brand-blue group-hover:text-white transition-colors duration-200 ml-px" />
        </div>
      </div>
...
function BoardPanel({ color, title, Icon, items }) {
  return (
    <div className="flex rounded-xl overflow-hidden border border-soft h-full">
      {/* Left header panel */}
      <div
        className={`flex flex-col items-center justify-center gap-5 px-6 py-10 text-white flex-shrink-0 bg-${color}`}
        style={{ minWidth: 140 }}
      >
      ...
      {/* Right list */}
      <div className="flex-1 px-5 py-4 bg-white overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <FadeInUp delay={0}>
          <div className="flex items-center gap-4 mb-8">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.28em] text-brand-blue whitespace-nowrap">
              Notices &amp; Resources
            </p>
            <div className="h-px flex-1 bg-soft" />
          </div>
